...
#if TARGET_ORBIS
extern void		InitCustomHeap	(size_t size);
#endif
...
inline __ai void _internal_memzero( void* dest )
{
	...
	switch (t_size)
	{
		...
		default:
			if (TARGET_ORBIS && (t_size < 512))
			{
				memset(dest, 0, t_size);
				return;
			}
...
#else
	__m128* d128 = (__m128*)dest;
	__m256* d256 = (__m256*)dest;
	__m128 zerox = _mm_setzero_ps();
	__m256 zero2 = _mm256_setzero_ps();

	switch (t_size)
	{
		// ORBIS Clang does a good job optimizing memset for very-short copies already.
		// It typically does a good job with longer copies > 128 bytes, using avx 256.
		// But often it fails to use AVX for ranges 64->128 -- and generates a comedy
		// of 64-bit mov [mem], 0 (very slow due to instruction length).
		...
		case 40:  {
			_mm256_storeu_ps( (float*)(d256+0), zero2 );
			((u64&)d256[1]) = 0;
		} break;
		...
		case 64:  {
			_mm256_storeu_ps( (float*)(d256+0), zero2 );
			_mm256_storeu_ps( (float*)(d256+1), zero2 );
		} break;

		case 80:  {
			_mm256_storeu_ps( (float*)(d256+0), zero2 );
			_mm256_storeu_ps( (float*)(d256+1), zero2 );
			i_movups( (__m128*)(d256+2), zerox );
		} break;

		case 96:  {
			_mm256_storeu_ps( (float*)(d256+0), zero2 );
			_mm256_storeu_ps( (float*)(d256+1), zero2 );
			_mm256_storeu_ps( (float*)(d256+2), zero2 );
		} break;
		...
		case 128:  {
			...
			_mm256_storeu_ps( (float*)(d256+1), zero2 );
			_mm256_storeu_ps( (float*)(d256+2), zero2 );
			_mm256_storeu_ps( (float*)(d256+3), zero2 );
		} break;
...
#if TARGET_ORBIS
		extern xString xFixFilenameForPS4( const xString& src );
#else
#		define xFixFilenameForPS4(src)		(src)
#endif

#if TARGET_ORBIS
	extern const char*		HOST_GetTemporaryDirectory		();
	extern void				Orbis_InitTemporaryDirectory	();
#endif
