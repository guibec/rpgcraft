...
inline __xi void xMemCopy( void* dest, const void* src, uint len )
{
    ...
    #if 0 && TARGET_ORBIS && !TARGET_DEBUG
    // Ok, so all of this code would have been awesome and clever, if orbis-clang
    // actually supported the __builtin_constant_p() feature, or supported it in
    // a matter that "works" anyway.  --jstine

    //bug_on_qa ((__builtin_constant_p(len) && len < 256) || (len < 1024));

    if (__builtin_constant_p(len)) __stop();
    if (__builtin_constant_p(dest)) __stop();
    if (__builtin_constant_p(src)) __stop();

    if (__builtin_constant_p(len)) {
        switch(len) {
            case 1:     *(u8*)dest  = *(u8*)src;            return;
            case 2:     *(u16*)dest = *(u16*)src;           return;
            case 4:     *(u32*)dest = *(u32*)src;           return;
            case 8:     *(u64*)dest = *(u64*)src;           return;

            case 12:    *(u64*)dest     = *(u64*)src;   
                        *((u32*)dest+2) = *((u32*)src+2);   return;

            case 16:    Copy128_Unaligned(dest, src);       return;
            case 20:    Copy128_Unaligned(dest, src);
                        *((u32*)dest+4) = *((u32*)src+4);   return;
            case 24:    Copy128_Unaligned(dest, src);
                        *((u64*)dest+2) = *((u64*)src+2);   return;
            case 28:    Copy128_Unaligned(dest, src);
                        *((u64*)dest+2) = *((u64*)src+2);
                        *((u32*)dest+6) = *((u32*)src+6);   return;

            case 32:    Copy128_Unaligned(dest, src);
                        Copy128_Unaligned((u8*)dest+16, (u8*)src+16); return;

            case 48:    Copy128_Unaligned(dest, src);
                        Copy128_Unaligned((u8*)dest+16, (u8*)src+16);
                        Copy128_Unaligned((u8*)dest+32, (u8*)src+32); return;

            case 64:    Copy128_Unaligned(dest, src);
                        Copy128_Unaligned((u8*)dest+16, (u8*)src+16);
                        Copy128_Unaligned((u8*)dest+32, (u8*)src+32);
                        Copy128_Unaligned((u8*)dest+48, (u8*)src+48); return;
        }
    }
    #endif
...
{
#if TARGET_ORBIS
    //bug_on_qa ((__builtin_constant_p(lenQwc) && lenQwc < 16) || (lenQwc < 64));
#endif
