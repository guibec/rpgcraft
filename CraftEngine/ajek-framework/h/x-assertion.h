...
// --------------------------------------------------------------------------------------
//                       PS4 (orbis) Stop/Trap/Debugbreak Policy
// --------------------------------------------------------------------------------------
// Policy for PS4 titles in Release Check mode is as follows:
//  # Avoid any usage of <libdbg.h>
//     * SCE_STOP() and SCE_BREAK() should not be present in any potential path of execution.
//  # Use __builtin_trap() to induce a crash & process dump.
//     * Due to security nature of the JIT, it is preferrable to crash the process in
//       situations where the system may be the subject of exploitation attempts.
//
...
#   else
#       define __stop()         ({ SCE_STOP(); })
#   endif
#else
    // Neither Windows nor Linux have a real concept of halting a system; but they do more-or-less
    // have a concept of sudden and brutal termination of the process no-questions-asked style.
...
#else
#   define      x_abort(...)                    (            (__stop())
#   define      x_abort_on(cond,...)            ( (cond) &&  (__stop(), true) )
#   define      abort_var(...)
#endif
