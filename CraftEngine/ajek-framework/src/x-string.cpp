...
#if TARGET_MSW || TARGET_ORBIS
	// PS4 also checks for MSW-style paths (C:\), as the Olympus file system internally
	// will append /host/ to these paths when detected.
...
#elif TARGET_ORBIS
	// PS4 lacks the helper templates present in msvc.
	char result[256];
	result[0] = 0;
