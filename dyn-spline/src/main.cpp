...
template< int _lenT >
constexpr u32 hashString(const char (&src)[_lenT])
{
	int srclen   =  _lenT;
	int srclen64 =  _lenT & ~7;
	int rem32    = (_lenT &  7) / 4;
	int rem16    = (_lenT &  3) / 2;
	int rem8     = (_lenT & ~1);
	...
	int i=0;
	for(; i<_lenT; i+=1) {
		result = i_crc32(result, (u8&)src[i]);
	}
	return result;
#if 0
	for(; i<srclen64; i+=8) {
		result = i_crc32(result, (u64&)src[i]);
	}

	if (rem32) {
		result = i_crc32(result, (u32&)src[i]);
		i += 4;
	}

	if (rem16) {
		result = i_crc32(result, (u16&)src[i]);
		i += 2;
	}
